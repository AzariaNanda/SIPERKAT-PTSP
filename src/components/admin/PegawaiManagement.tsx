...
export const PegawaiManagement = () => {
  ...
  const handleAddEmail = async () => {
    ...
    try {
      // REVISI: is_registered tetap FALSE saat diinput admin (Status: Belum Daftar)
      ...
      if (error) throw error;
      toast.success("Pegawai berhasil ditambahkan ke whitelist");
      setNewEmail('');
      fetchWhitelist();
    } catch (error: any) {
  ...
  const handleBulkAdd = async () => {
    ...
    if (emailArray.length === 0) return toast.error("Tidak ada email valid");
    ...
    try {
      ...
      // Upsert: Jika email sudah ada, abaikan (onConflict: email)
      const { error } = await supabase.from('pegawai_whitelist').upsert(dataToInsert, { onConflict: 'email' });
      if (error) throw error;

      toast.success(`${emailArray.length} Email diproses.`);
      setBulkEmails('');
      fetchWhitelist();
    } catch (error) {
  ...
  const toggleRole = async (email: string, currentRole: string) => {
    if (email === MAIN_ADMIN_EMAIL) return toast.error("Admin Utama tidak bisa diubah");
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        <Card className="border-none shadow-xl bg-white rounded-3xl overflow-hidden lg:col-span-1">
          ...
          <CardContent className="p-5 space-y-4">
            <Input placeholder="email@gmail.com" value={newEmail} onChange={(e) => setNewEmail(e.target.value)} className="h-12" />
        ...
        <Card className="border-none shadow-xl bg-white rounded-3xl overflow-hidden lg:col-span-2 border-l-4 border-primary">
          ...
          <CardContent className="p-5 space-y-3">
            ...
            <Button onClick={handleBulkAdd} disabled={isSubmitting} variant="default" className="w-full h-12 font-black uppercase shadow-lg shadow-primary/10">Proses Semua</Button>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="border-none shadow-2xl bg-white overflow-hidden rounded-3xl">
        <CardHeader className="bg-slate-50 p-6 border-b flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <CardTitle className="text-lg font-black uppercase tracking-tighter text-slate-800">Daftar Whitelist Pegawai</CardTitle>
          </div>
          <div className="flex gap-2">
            ...
            <Button variant="outline" size="icon" onClick={fetchWhitelist} className="rounded-xl border-slate-200"><RefreshCcw className="w-4 h-4" /></Button>
          </div>
        </CardHeader>
